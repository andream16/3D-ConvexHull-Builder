...
void DcelManager::resetDefaults() {
    ui->wireframeDcelCheckBox->setChecked(true);
    ui->drawDcelCheckBox->setChecked(true);
    ui->pointsDcelRadioButton->setChecked(true);
...
void DcelManager::setButtonsDcelLoaded() {
    ...
    ((ConvexHullManager*)mainWindow->getManager(CONVEX_HULL_MANAGER_ID))->enableCHButton();
}
...
void DcelManager::setButtonsDcelNotLoaded() {
    ...
    ((ConvexHullManager*)mainWindow->getManager(CONVEX_HULL_MANAGER_ID))->disableCHButton();
}
/**
    ...
    if (!filename.isEmpty())
    {
        ...
        if(s.substr(s.find_last_of(".") + 1) == "obj") { //obj file
            ...
            if (! drawableDcel->isTriangleMesh()){
                drawableDcel->triangulate();
                std::cout << "No Triangle Mesh\n";
            }
            else
...
void DcelManager::on_saveDcelButton_clicked() {
    ...
    QString filename = QFileDialog::getSaveFileName(nullptr,
                       "Save DCEL",
                       ".",
                       "PLY(*.ply);;OBJ(*.obj);;DCEL(*.dcel)", &selectedFilter);
...
void DcelManager::on_flatDcelRadioButton_toggled(bool checked) {
    if (checked){
        drawableDcel->setFlatShading();
        if (ui->triangleColorDcelRadioButton->isChecked()) drawableDcel->setEnableTriangleColor();
        if (ui->vertexColorDcelRadioButton->isChecked()) drawableDcel->setEnableVertexColor();
    }
    mainWindow->updateGlCanvas();
}
